...
1. The parent template's `copy.sh` copies `sb-sandbox.env` with `SB_IMAGE=sb-ubuntu-noble:latest`
...
```bash
SB_TEMPLATE_ID="sb-ubuntu-noble-fw"
SB_IMAGE=sb-ubuntu-noble-fw:latest
SB_SHELL=/bin/bash
#SB_MODULES=("example")
```

Key differences from parent (`sb-ubuntu-noble/artifacts/sb-sandbox.env`):
- `SB_TEMPLATE_ID="sb-ubuntu-noble-fw"` (was `"sb-ubuntu-noble"`)
- `SB_IMAGE=sb-ubuntu-noble-fw:latest` (was `sb-ubuntu-noble:latest`)

### Step 2: Update copy.sh to unconditionally set SB_IMAGE

Modify file: `templates/sandboxes/sb-ubuntu-noble-fw/hooks/create/copy.sh`

Current line 47:
```bash
sed -i 's/SB_IMAGE=sb-ubuntu-noble/SB_IMAGE=sb-ubuntu-noble-fw/' $new_sandbox_path/sb-sandbox.env
```

Replace with:
```bash
sed -i 's/^SB_IMAGE=.*/SB_IMAGE=sb-ubuntu-noble-fw:latest/' $new_sandbox_path/sb-sandbox.env
```

This change:
- Uses `^SB_IMAGE=.*` to match any existing `SB_IMAGE` line regardless of its current value
...
2. **Test sandbox creation** (if test project available):
   ...
   cat .sb/sandboxes/default/sb-sandbox.env | grep SB_IMAGE
   ```
   Expected: `SB_IMAGE=sb-ubuntu-noble-fw:latest`
...
2. **Robust sed pattern**: The `^SB_IMAGE=.*` pattern works regardless of what value the parent set
