    ...
    for mod in "${modules[@]}"; do
      ...
      if [ -f "$shell_login_hook_script" ]; then
        ...
        if grep -q -E -i '^(true)|(1)$' <<< "${SB_HOOK_DEBUG_ENABLED}"; then
          echo "$SCRIPT_NAME: Module [$mod] Sourcing $shell_login_hook_script"
        fi

        login_cmds=$($shell_login_hook_script)
        if [ $? -ne 0 ]; then
        ...
        if [ $? -ne 0 ]; then
          echo "$SCRIPT_NAME: Module [$mod] WARNING- generated commands execution of $shell_login_hook_script failed"
        else
          if grep -q -E -i '^(true)|(1)$' <<< "${SB_HOOK_DEBUG_ENABLED}"; then
            echo "$SCRIPT_NAME: Module [$mod] $shell_login_hook_script generated commands execution successful"
          fi
        fi        
